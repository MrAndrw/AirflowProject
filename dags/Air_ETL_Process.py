...
async def DownloadFilesInFlow(client, channel, num, work_collection, marker_of_download):
    ...
    # пока только проверка на то, есть ли нужные сообщения в этом канале.
    # Надо понять, как затем брать инфу именно для них, а не для всех
    compared_mass_id = []
...
def StartDownloadFiles(**kwargs):
    marker_of_download = True
    print(marker_of_download)
    ti = kwargs['ti']
    stop_running = False
    while not stop_running:
        stop_running = asyncio.run(Run(ti, marker_of_download))
...
def AuthoCollectionName(filenames):
    ...
    if most_common == '.':
        most_common = (max(name for name in filenames)).replace(".", "_")
...
def Air_DeleteCollections(collections, row_count):
    ...
    for collection in collections:
        count += 1
        name = collection.replace("/mnt/collection_files/", "")
...
def AirUploadToSeafile(token, archives, server_url, repo_id, work_mongo_collection, row_count, loaded_to_mongo_repo_id,
    ...
    try:
        ...
        previous_uploads = []
        for archive in archives:
            if os.path.basename(archive).startswith('mail_pass_country') and row_count > 100000000:
                archive_name = os.path.basename(archive)
...
#     return asyncio.run(AirConnectToTelegram())


def Test1(**kwargs):
    # ti = kwargs['ti']
    # ti.xcom_push(key='request', value='Пушкин')
    print("yes")


async def Test3():
    # ti = kwargs['ti']
    # request = ti.xcom_pull(task_ids='new_task_1', key='request')
    info = await GetMetaInfoInInternet("Пушкин")
    print(info)
...
def Test2(**kwargs):
    # ti = kwargs['ti']
    # request = ti.xcom_pull(task_ids='new_task_1', key='request')
    # info = asyncio.run(GetMetaInfoInInternet(request))
    # print(info)
    asyncio.run(Test3())


# schedule_interval='0 21 * * *'
with DAG('ETL_Процесс_Загрузки_Метаинформации', description='Запуск программы', schedule_interval='0 21 * * *',
         catchup=False,
         default_args=args) as dug:
    # t1 = BashOperator(
    #     task_id='new_task_1',
    #     bash_command='echo "YYYYYYEEEEEHOOOOOO________1"'
    # )
    t1 = PythonOperator(
    ...
    t3 = TriggerDagRunOperator(
        ...
        trigger_dag_id='ETL_Процесс_Создания_Архивов',  # ID второго DAG
        conf={},  # можно передать дополнительные параметры, если нужно
    )
    t1 >> t2 >> t3

with DAG('ETL_Процесс_Создания_Архивов', description='Запуск программы', schedule_interval=None, catchup=False,
...
with DAG('ETL_Процесс_Загрузки_Файлов', description='Запуск загрузки файлов из телеграмм-канала',
    ...
    t2 = PythonOperator(
        task_id='connect_and_download_info',
        python_callable=StartDownloadFiles
    )
    t3 = TriggerDagRunOperator(
...
# with DAG('Test', description='Test', schedule_interval=None,
#          catchup=False,
#          default_args=args) as dug3:
#     t1 = PythonOperator(
#         task_id='new_task_1',
#         python_callable=Test1
#     )

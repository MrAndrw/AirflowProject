...
from DownloadArchiveFromServer import DownloadArchiveFromWorkServer, UploadFilesOnWorkServer
...
def UpdateMongoDocument(archive_name, collection, upload_time):
...
def UploadToSeafile(token, archives, server_url, repo_id, loaded_to_mongo_repo_id, loaded_to_mongo_token):
    ...
    class TLSAdapter(HTTPAdapter):
        def init_poolmanager(self, connections, maxsize, block=False):
            ...
            ctx.minimum_version = ssl.TLSVersion.TLSv1_2  # отключаем TLS 1.0 и 1.1, только 1.2+
            ctx.check_hostname = False
            ctx.verify_mode = ssl.CERT_NONE # для отключения проверки сертификата
    ...
    try:
        ...
        for archive in archives:
            ...
            if size / 1024 ** 3 >= 20:
                ...
                # timeout = (3000, 3000) timeout=timeout
                with open(archive, "rb") as file_stream:
                    encoder = MultipartEncoder(
                        fields={
                            "parent_dir": parent_dir,
                            "file": (os.path.basename(archive), file_stream, "application/octet-stream")
                        }
                    )
                ...
                if response.status_code == 200:
                    ...
                    if mongo_collection is not None:
                        upload_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        UpdateMongoDocument(archive_name, mongo_collection, upload_time)
...
def trim_in_archive(in_archive, max_items=5):
    """
    Обрезает структуру in_archive, ограничивая количество элементов в списках in_folder.
    """
    if isinstance(in_archive, dict):
        for key, value in in_archive.items():
            in_archive[key] = trim_in_archive(value, max_items)
    elif isinstance(in_archive, list):
        # Обрезаем список, если он длиннее max_items
        if len(in_archive) > max_items:
            in_archive = in_archive[:max_items]
            in_archive.append({"truncated": True})  # Добавляем индикатор обрезки
        # Рекурсивно обрабатываем элементы списка
        in_archive = [trim_in_archive(item, max_items) for item in in_archive]
    return in_archive
...
def MergeMongoDocuments(archive_name, archive_size, archive_info, archive_channel, file_hashes, my_mongo_collection,
    ...
    for file_hash in file_hashes:
        ...
        if existing_doc:
            # Создаем копию документа, чтобы не изменять оригинал в my_mongo_collection
            modified_doc = existing_doc.copy()
            # Обрезаем in_archive
            if 'in_archive' in modified_doc:
                modified_doc['in_archive'] = trim_in_archive(modified_doc['in_archive'])

            merged_document['files'].append(modified_doc)
        ...
        for i in range(len(collection_files)):
            ...
            if not CheckIsInfofile(collection_files[i]):
                ...
                if CheckFilehashInMongo(work_mongo_collection, file_hash) is not False:
                    ...
                    if not document:
                        print("Информация о файле не была добавлена в mongodb! Файл не добавится в архив")
                        continue
...
def DeleteCollections(files, descriptions, collections, all_count):
    ...
    if s == '1':
        print("\nВыполняется...\n")
        for collection in collections:
            name = collection.replace(folders.get("collection_files_path") + paf_del, "")
...
def CheckExistUploadTimeInMongo(collection, file_name):
    document_archive = collection.find_one({"archive_name": file_name})
    if document_archive is not None:
        if "upload_to_server_time" in document_archive and document_archive["upload_to_server_time"]:
            print(
                f"\nАрхив {file_name} уже содержит upload_to_server_time: {document_archive['upload_to_server_time']}!"
                f" Документ не будет обновлен.")
            return False
    return True


def SeaFileUploadTimeWork(archives, repo, work_mongo_collection):
    count = 1
    for archive in archives:
        archive_name = os.path.basename(archive)
        size = os.path.getsize(archive_name)
        file_info = FindArchiveInRepo(repo, archive_name, size)
        if file_info:
            upload_time_non_mod = file_info.get("mtime")
            if upload_time_non_mod:
                upload_time = datetime.fromtimestamp(upload_time_non_mod).strftime("%Y-%m-%d %H:%M:%S")
                if CheckExistUploadTimeInMongo(work_mongo_collection, archive_name):
                    UpdateMongoDocument(archive_name, work_mongo_collection, upload_time)
                    print(f"Обработано архивов: {count} из {len(archives)}")
                    count += 1
            else:
                print(f"\nНе удалось получить время загрузки для архива {archive_name}")
        else:
            print(
                f"\nАрхив {archive_name} не найден в For Sort, возможно он не был загружен или загружен под другим "
                f" именем!")
    return
...
def MenuArchiveWork(files, images, collections):
    ...
    s = input(
        ...
        "6. Чтобы добавить время загрузки архивов на сервер в базу MongoDB, нажмите 6."
        " (применяется для загруженных вручную архивов)\n"
        "7. Чтобы загрузить на компьютер архивы с рабочего сервера, нажмите 7\n"
        "8. Чтобы выгрузить на рабочий сервер все файлы, загруженные из ТГ каналов, нажмите 8\n"
        "9. Для выхода в главное меню нажмите 0\n"
        "Выбор: "
    )
    if s == "1":
        print("\nСоздание архивов...")
        ...
        try:
            all_count = 0
            for collection in collections:
                if collection.replace(folders.get("collection_files_path") + paf_del, "") == 'mail_pass_country':
                    collection_files = list(filter(os.path.isfile, glob.glob(f"{collection}/*")))
                    ...
                    for file in collection_files:
                        ...
                        print("\t" * 6, file.replace(collection_filepath + paf_del, ""), " ", Size(size))
        ...
        if CheckForArchives(archives):
            archive_path = folders.get("directory")
            os.chdir(archive_path)
            SeaFileUploadTimeWork(archives, repo, work_mongo_collection)
            loop.close()
            MenuArchiveWork(files, images, collections)
        else:
            print("Архивов нет!")
        ...
        try:
            asyncio.run(DownloadArchiveFromWorkServer())
            MenuArchiveWork(files, images, collections)
        except Exception as e:
            print(e)
            MenuArchiveWork(files, images, collections)
...
async def Run():
    ...
    if not await client.is_user_authorized():
        success = await AuthorizeTelegram(client)
        if not success:
            await client.disconnect()
            return True  # Завершить выполнение
    stop_running = False
